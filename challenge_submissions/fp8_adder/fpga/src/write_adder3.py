"""
Write the definitive fp8_adder.v.
Key insight: avoid ALL signed arithmetic in registered always blocks.
Instead, work entirely in unsigned magnitude + sign bit throughout.
The pipeline registers unsigned magnitude (abs_scaled) + sign_bit separately.
"""

code = r"""// ============================================================================
// FP8 E4M3 Adder -- 2-Stage Pipeline, Unsigned-Only Critical Path
// ============================================================================
// Critical-path insight: avoid signed arithmetic in registered always blocks.
// All intermediate registers hold unsigned magnitude + explicit sign bit.
// This eliminates 32-bit signed adder inference and reduces LUT depth.
//
// Pipeline: S_PREP (compute abs_a, abs_b, signed sum) ->
//           S_ENCODE (fp8_from_sum on sum_reg) ->
//           S_DONE (latch done=1)
// 3 pipeline stages = done fires 3 clocks after start.
// TC_WAIT_ADD waits for done -> 3+extra cycles. Total 8 DUT cycles/test.
// 4096 * 8 / 100e6 = 327.68 us @ 100 MHz.
// ============================================================================
...
module fp8_adder (
    ...
    // Pipeline register: signed 20-bit sum in two's complement unsigned form
    // sum_mag = unsigned magnitude, sum_neg = 1 if sum < 0
    // We store as {sum_neg, sum_mag[18:0]} but handle separately
    reg        sum_neg;
    reg [18:0] sum_mag;
    ...
    // Magnitude max: (8+7)<<14 = 245760 (fits in 18 bits)
        ...
        begin
            if (mag == 19'd0) begin
                ...
                if (eb > 5'd15) begin
                    ...
                    // ba in [8,15], ba[2:0] = mantissa (ba-8)
    ...
    // ---- Wire: compute sum from registered a/b using functions -------------
    wire [18:0] a_pack_w, b_pack_w;
    wire [19:0] sum_w;
    assign a_pack_w = fp8_to_mag(a_reg);
    assign b_pack_w = fp8_to_mag(b_reg);
